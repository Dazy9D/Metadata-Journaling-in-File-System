...
int main(int argc, char *argv[])
{
    ...
    // Journal block 0: header only, marking the journal empty.
    // "Empty" per the spec means nbytes_used == sizeof(journal_header).
    printf("Initializing journal area...\n");
    memset(block, 0, sizeof(block));
    struct journal_header *jh = (struct journal_header *)block;
    jh->magic = JOURNAL_MAGIC;
    jh->nbytes_used = sizeof(struct journal_header);
    write_block(fd, block); // Journal header block

    // Remaining journal blocks start out zeroed/unused.
    memset(block, 0, sizeof(block));
